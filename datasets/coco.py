...
class COCO(data.Dataset):
    ...
    """
        ...
        if len(bboxes) == 0:
            bboxes = np.array([[0.0, 0.0, 0.0, 0.0]], dtype=np.float32)
            labels = np.array([[0]])
        bboxes[:, 2:] += bboxes[
            :, :2
        ]  # xywh to xyxy, top-left x, top-left y, bottom-right x, bottom-right y
...
class COCO_eval(COCO):
    def __init__(
        self, data_dir, split, test_scales=(1,), test_flip=False, fix_size=False
    ):
